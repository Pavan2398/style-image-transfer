...
# Silence TensorFlow/oneDNN logs as early as possible (must run before TF import).
...
from segmentation import get_regions, visualize_regions
...
USE_REGIONS = False
REGIONS_K = 3
REGION_STYLE_WEIGHTS = None  # default: equal weights
SAVE_REGIONS = False
...
def build_parser():
    ...
    # --- Modern optional improvements (all off by default) ---
    parser.add_argument(
        "--use-saliency",
        action="store_true",
        dest="use_saliency",
        default=USE_SALIENCY,
        help="enable saliency-aware style transfer (reduce style in salient regions)",
    )
    parser.add_argument(
        "--saliency-weight",
        type=float,
        dest="saliency_weight",
        default=SALIENCY_WEIGHT,
        help="weight for saliency preservation loss (default %(default)s)",
    )
    parser.add_argument(
        "--save-saliency",
        action="store_true",
        dest="save_saliency",
        default=SAVE_SALIENCY,
        help="save computed saliency map next to OUTPUT",
    )
    ...
    parser.add_argument(
        ...
        dest="use_adain",
        default=USE_ADAIN,
        help="use AdaIN-style mean/std matching as style loss (instead of Gram matrices)",
    )

    parser.add_argument(
        "--use-patch-style",
        action="store_true",
        dest="use_patch_style",
        default=USE_PATCH_STYLE,
        help="enable lightweight patch-based style matching loss (can be slow/memory heavy)",
    )
    parser.add_argument(
        "--patch-weight",
        type=float,
        dest="patch_weight",
        default=PATCH_WEIGHT,
        help="weight for patch-based style loss (default %(default)s)",
    )
    ...
    parser.add_argument(
        ...
        dest="use_regions",
        default=USE_REGIONS,
        help="enable region-aware style transfer using simple color clustering",
    )
    parser.add_argument(
        "--regions-k",
        type=int,
        dest="regions_k",
        default=REGIONS_K,
        help="number of regions for --use-regions (default %(default)s)",
    )
    parser.add_argument(
        "--region-style-weights",
        type=float,
        dest="region_style_weights",
        nargs="+",
        default=REGION_STYLE_WEIGHTS,
        help="optional per-region weights (provide K floats; default: equal)",
    )
    parser.add_argument(
        "--save-regions",
        action="store_true",
        dest="save_regions",
        default=SAVE_REGIONS,
        help="save a visualization of the computed regions next to OUTPUT",
    )
    return parser
...
def main():
    ...
    # Optional: saliency + regions computed from the (possibly resized) content image
    saliency_mask = None
    if options.use_saliency:
        saliency_mask = get_saliency_map(np.clip(content_image, 0, 255).astype(np.uint8))
        if options.save_saliency:
            _save_mask_image(_default_mask_path(options.output, "_saliency"), saliency_mask)

    region_masks = None
    region_style_weights = None
    if options.use_regions:
        region_masks = get_regions(np.clip(content_image, 0, 255).astype(np.uint8), k=int(options.regions_k))
        if options.region_style_weights is None:
            region_style_weights = [1.0 / region_masks.shape[2] for _ in range(region_masks.shape[2])]
        else:
            if len(options.region_style_weights) != region_masks.shape[2]:
                parser.error("--region-style-weights must have exactly K values (matching --regions-k)")
            s = float(sum(options.region_style_weights))
            region_style_weights = [float(w) / s for w in options.region_style_weights]
        if options.save_regions:
            regions_vis = visualize_regions(region_masks)
            imsave(_default_mask_path(options.output, "_regions", ext=".jpg"), regions_vis.astype(np.float32))
    ...
    for iteration, image, loss_vals in stylize(
        ...
        use_saliency=options.use_saliency,
        saliency_mask=saliency_mask,
        saliency_weight=options.saliency_weight,
        use_adain=options.use_adain,
        use_patch_style=options.use_patch_style,
        patch_weight=options.patch_weight,
        use_regions=options.use_regions,
        region_masks=region_masks,
        region_style_weights=region_style_weights,
...
def _save_mask_image(path: str, mask_01: np.ndarray) -> None:
    """
    Save a 2D mask (H,W) in [0,1] as a grayscale PNG/JPG depending on extension.
    """
    mask = np.clip(mask_01 * 255.0, 0, 255).astype(np.uint8)
    Image.fromarray(mask, mode="L").save(path, quality=95)


def _default_mask_path(output_path: str, suffix: str, ext: str = ".png") -> str:
