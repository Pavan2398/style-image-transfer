...
# Silence TensorFlow logs as early as possible (must run before TF import).
...
# Reduce Python-level TF logging noise.
try:
...
def _pool_layer(input, pooling):
    if pooling == "avg":
        return tf.nn.avg_pool2d(input, ksize=2, strides=2, padding="SAME")
    else:
        return tf.nn.max_pool2d(input, ksize=2, strides=2, padding="SAME")
